      ...
      <div className="container mx-auto max-w-7xl relative z-10">
        ...
        >
          <div className="flex flex-col md:flex-row gap-4">
            <SearchFilter searchQuery={searchQuery} onSearchChange={setSearchQuery} placeholder="Search articles..." placeholderVn="Tìm kiếm bài viết..." />
            <CategoryFilter categories={blogCategories} activeCategory={activeCategory} onCategoryChange={setActiveCategory} />
          </div>
        </motion.div>
          ...
          <motion.div className="mb-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {featuredPosts.map((post, i) => (
                <BlogCard key={post.id} post={post} index={i} isHovered={false} onHover={() => {}} variant="featured" />
              ))}
            </div>
          </motion.div>
        )}
        ...
        <div className="mb-4 text-xs text-muted-foreground">
          {t(language, `Showing ${filteredPosts.length} articles`, `Hiển thị ${filteredPosts.length} bài viết`)}
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          <AnimatePresence mode="popLayout">
            {filteredPosts.map((post, i) => (
              <BlogCard key={post.id} post={post} index={i} isHovered={hoveredPost === post.id} onHover={setHoveredPost} />
            ))}
          </AnimatePresence>
        </div>
      </div>
