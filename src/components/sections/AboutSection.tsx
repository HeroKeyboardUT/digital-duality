...
const techData = [
  { category: 'LANGUAGES', items: ['C++', 'C', 'Python', 'Java', 'JavaScript', 'TypeScript'] },
...
export function AboutSection() {
  ...
  return (
    <section id="about" className={`py-20 px-4 border-t-2 border-foreground ${theme === 'dark' ? 'hex-pattern' : ''}`}>
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-6">
            ...
            >
              <div className="label-text mb-3">{t(language, 'EDUCATION', 'HỌC VẤN')}</div>
              <h3 className={`font-sans font-bold text-sm mb-2 ${theme === 'dark' ? 'text-flicker' : ''}`}>
                VNU-HCM University of Technology
              </h3>
              <div className={`text-xs mb-2 ${theme === 'dark' ? 'text-accent' : 'text-muted-foreground'}`}>
            ...
            <div className="space-y-4">
              <div className="label-text">{t(language, 'SKILLS', 'KỸ NĂNG')}</div>
              {skills.map((skill, i) => (
                <motion.div
                  key={skill.category}
                  className={`grid-cell ${theme === 'dark' ? '' : ''}`}
                  initial={{ opacity: 0, x: -20 }}
                  whileInView={{ opacity: 1, x: 0 }}
                  viewport={{ once: true }}
                  transition={{ delay: i * 0.05 }}
                >
                  <h4 className={`font-sans font-bold text-xs mb-1 ${theme === 'dark' ? 'text-accent' : ''}`}>
          ...
          {/* Right: Tech Stack */}
          <motion.div
            className={`grid-cell h-fit ${theme === 'dark' ? 'glow-border' : ''}`}
            initial={{ opacity: 0, x: 20 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
          >
