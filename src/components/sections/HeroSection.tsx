...
import { ArrowDown, Github, Linkedin, Mail, Phone, Sparkles, Zap } from 'lucide-react';
...
export function HeroSection() {
  ...
  return (
    <section id="hero" className="min-h-[90vh] flex flex-col justify-center relative py-20 px-4 overflow-hidden">
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        <div className="grid lg:grid-cols-[1fr,auto] gap-12 items-center">
          ...
          <div>
            ...
            >
              <span className={`label-text ${theme === 'dark' ? 'text-flicker' : ''}`}>
            ...
            >
              ...
              <div className={`text-lg sm:text-xl font-mono flex items-center gap-2 ${theme === 'dark' ? 'text-primary cursor-blink' : ''}`}>
                {theme === 'dark' && <Zap size={16} className="text-accent animate-pulse" />}
                {t(language, 'Pham Quang Hieu', 'Phạm Quang Hiếu')}
                {theme === 'light' && <Sparkles size={16} className="text-muted-foreground" />}
              </div>
            </motion.div>
            ...
            >
              <h2 className={`text-xl sm:text-2xl font-sans font-bold mb-4 ${theme === 'dark' ? 'typing-effect' : ''}`}>
              ...
              <p className={`text-sm leading-relaxed max-w-md ${theme === 'dark' ? 'text-muted-foreground' : ''}`}>
                {t(language,
              ...
              <div className={`${theme === 'dark' ? 'grid-cell glow-border' : 'grid-cell hover-lift'} mt-6`}>
        ...
        {/* Stats Bar */}
        <motion.div
          className={`grid grid-cols-2 sm:grid-cols-4 gap-4 border-t-2 border-foreground pt-8 mt-8 ${theme === 'dark' ? 'pulse-border' : ''}`}
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          {[
            ...
            <motion.div 
              key={i} 
              className={`text-center sm:text-left ${theme === 'dark' ? 'hover:scale-105' : 'hover-lift'}`}
              whileHover={{ scale: theme === 'dark' ? 1.05 : 1 }}
      ...
      >
        <ArrowDown size={24} className={theme === 'dark' ? 'text-primary neon-text' : ''} />
      </motion.div>
    </section>
  );
}
