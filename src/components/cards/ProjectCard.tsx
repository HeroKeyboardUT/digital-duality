...
import { Github, ExternalLink, FileText, Play, Sparkles, ChevronRight } from 'lucide-react';
      ...
      <div className="flex gap-2 flex-wrap">
        {project.links?.github && (
          <motion.a
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold border-2 border-foreground transition-all ${
              theme === 'dark' ? 'hover:border-accent hover:text-accent hover:shadow-[0_0_10px_hsl(var(--neon-cyan)/0.3)]' : 'hover:bg-foreground hover:text-background'
            }`}
        ...
        {project.links?.live && (
          <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
            ...
            >
              <ExternalLink size={12} /> {t(language, 'Live Demo', 'Xem Demo')}
            </Link>
          </motion.div>
        )}
        {project.links?.pdf && (
          <motion.a
            ...
            rel="noopener noreferrer"
            className={`flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold border-2 border-foreground transition-all ${
              theme === 'dark' ? 'hover:border-primary hover:text-primary' : 'hover:bg-muted'
            }`}
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
          >
