    ...
    return (
      <motion.article
        className={`group p-5 border-2 border-foreground relative overflow-hidden ${
          theme === "dark"
            ? "glow-border bg-gradient-to-br from-primary/5 to-accent/5"
            : "bg-muted/30"
        }`}
      ...
      >
        <span
          className={`absolute top-3 right-3 text-2xl font-black opacity-20 ${theme === "dark" ? "text-accent" : ""}`}
        >
          {post.icon}
        </span>
        <div
          className={`text-[10px] px-2 py-0.5 inline-block border mb-2 ${theme === "dark" ? "border-accent text-accent" : "border-foreground"}`}
        >
          {post.category}
        </div>
        <h3
          className={`font-bold text-lg mb-2 group-hover:translate-x-1 transition-transform ${theme === "dark" ? "group-hover:text-accent" : ""}`}
        >
    ...
    >
      ...
      <div className="flex justify-between items-start mb-3 relative">
        <div className="flex items-center gap-2">
          <span
            className={`text-[9px] px-2 py-1 border-2 font-bold ${theme === "dark" ? "border-primary text-primary" : "border-foreground/50"}`}
          >
            {post.category}
          </span>
          {post.featured && (
            <span
              className={`text-[9px] flex items-center gap-1 ${theme === "dark" ? "text-accent" : ""}`}
            >
              <Sparkles size={8} />
            </span>
          )}
      ...
      {/* Title */}
      <h2
        className={`font-sans font-bold text-lg mb-2 group-hover:translate-x-1 transition-transform ${theme === "dark" ? "text-flicker" : ""}`}
      >
        <Link
          to={`/blog/${post.id}`}
