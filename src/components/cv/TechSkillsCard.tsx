    ...
    >
        ...
        {cvSkills.map((category, idx) => (
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              <div className={`w-2 h-2 ${category.color === 'primary' 
                ? theme === 'dark' ? 'bg-primary' : 'bg-foreground' 
                : theme === 'dark' ? 'bg-accent' : 'bg-muted-foreground'}`} 
              />
      ...
      <div className="mt-6 pt-4 border-t border-foreground/10">
        ...
        <div className="space-y-2">
          {proficiencyLevels.map((item, idx) => (
            <motion.div
              key={idx}
              initial={{ opacity: 0 }}
            ...
            >
              <div className="flex justify-between text-xs mb-1">
                <span>{language === 'en' ? item.skill : item.skillVn}</span>
                <span className="text-muted-foreground">{item.level}%</span>
              </div>
              <div className={`h-1.5 ${theme === 'dark' ? 'bg-secondary' : 'bg-muted'}`}>
                <motion.div
                  className={`h-full ${theme === 'dark' ? 'bg-gradient-to-r from-primary to-accent' : 'bg-foreground'}`}
                  initial={{ width: 0 }}
