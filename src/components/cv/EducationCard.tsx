    ...
    >
      ...
      <div className="space-y-3 relative">
        ...
        <div className="flex items-center gap-4 text-xs text-muted-foreground">
          ...
          <span className="flex items-center gap-1">
            <Award size={12} /> {t(language, "Year 3", "Năm 3")}
          </span>
        </div>
        ...
        >
          ...
          <div>
            <div className="text-xs text-muted-foreground uppercase">
              {t(language, "Current GPA", "GPA Hiện Tại")}
            </div>
            <motion.div
          ...
          <div
            className={`ml-auto px-2 py-1 text-[10px] font-bold uppercase
            ${theme === "dark" ? "bg-accent/20 text-accent" : "bg-foreground text-background"}`}
          >
            {t(language, "8.28/10.0", "8.28/10.0")}
          </div>
        </motion.div>

        {/* Achievements */}
        <div className="flex flex-wrap gap-2">
          {[{ text: "Active Learner", textVn: "Tự Học" }].map((item, idx) => (
            <motion.span
              key={idx}
